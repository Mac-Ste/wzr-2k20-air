...
void DrawScene()
{
	GLfloat OwnVehicleColor[] = { 0.4f, 0.2f, 0.8f, 0.5f };
	GLfloat AlienVehiclesColor[] = { 0.6f, 0.6f, 0.35f, 0.7f };
	//GLfloat RedSurface[] = { 0.8f, 0.2f, 0.1f, 0.5f };
	
	GLfloat YellowSurface[] = { 0.35f, 0.85f, 0.1f, 1.0f };
	

	GLfloat LightAmbient[] = { 0.1f, 0.5f, 0.1f, 0.1f };
	GLfloat LightDiffuse[] = { 0.4f, 0.7f, 0.7f, 0.7f };
	GLfloat LightPosition[] = { 5.0f, 5.0f, 7.0f, 0.0f };
	...
	float angle = (float)x / (time_of_day*CLOCKS_PER_SEC) * 2 * 3.1415926535898;
	//char lan[128];
	//sprintf(lan,"angle = %f\n", angle);
	...
	GLfloat GroundSurface[] = { 0.6*(0.8 + 0.4*sin_angminpi), 0.5*(0.2 + 0.8*sin_angminpi), 0.4*(0.1 + 0.9*sin_angminpi), 1.0f };
	...
	for (int w = -1; w < 2; w++)
		for (int k = -1; k < 2; k++)
		{
			...
			//for each (auto&& it in network_vehicles) if(it.second) it.second->DrawObject();   // od VC 2013 (C++ 11)
