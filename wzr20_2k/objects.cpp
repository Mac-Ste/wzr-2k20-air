...
MovableObject::MovableObject()             // konstruktor                   
{
	...
	// sta³e samochodu
	mass_own = 10;// 8.0 + 8.0*(float)rand() / RAND_MAX;			// masa obiektu [kg]
	...
	friction_linear = 2.1;// 1.5 + 3.0*(float)rand() / RAND_MAX;              // wspó³czynnik tarcia obiektu o pod³o¿e 
...
{                                                // dzia³aj¹cych si³ i czasu, jaki up³yn¹³ od ostatniej symulacji
	...
	float Fy = mass_own*g;        // si³a nacisku na podstawê obiektu (na ko³a pojazdu)
...
Terrain::Terrain()
{
	field_size = 27;         // d³ugoœæ boku kwadratu w [m]           
	...
	for (long i = 0; i < number_of_rows * 2 + 1; i++)
		for (long j = 0; j < number_of_columns + 1; j++)
			height_map[i][j] = 0.4*height_map[i][j] * exp(-0.3*log(1.1 + fabs(height_map[i][j])))*(5 + 3 * fabs(sin(13 * (float)i / number_of_rows)) + 2 * fabs(sin(11 * (float)j / number_of_columns)));
