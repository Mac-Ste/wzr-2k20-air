...
FILE *f = fopen("WzrWWC-log.txt", "a"); // plik do zapisu informacji testowych
...
long time_of_day = 2000;         // czas trwania dnia w [s]
...
void VirtualWorldCycle()
{
	...
	{                              // nale¿y na nowo obliczyæ œredni czas cyklu fDt
		...
		sprintf(text, "WZR/WWC 2019/20, tem.2, wer.k (jak Klaudiusz), czêstoœæ pr.wys. = %0.2f[r/s]  œr.odl = %0.3f[m]  œr.ro¿n.k¹t. = %0.3f[st]",
		...
		//sprintf(text, "WWC-lab 2018/19 temat 2, wersja o (%0.0f fps  %0.2fms) ", fFps, 1000.0 / fFps);
		if (time_from_start_in_s > 6)
	...
	int       nCmdShow)
{
	...
	window_handle = CreateWindow(class_name, "WWC/WZR-lab 2019/20 temat 2 - Nawigacja obliczeniowa, wersja k (jak Klaudiusz)", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
		50, 50, 900, 650, NULL, NULL, hInstance, NULL);
